...
impl MobileCircuit {
    pub fn new_ecdsa() -> Self {
        Self { circuit_type: CircuitType::ECDSA }
    }
    
    pub fn new_jwt() -> Self {
        Self { circuit_type: CircuitType::JWT }
    }
}

impl SpartanCircuit<E> for MobileCircuit {
    fn synthesize<CS: ConstraintSystem<Scalar>>(
        ...
        let (r1cs_file, witness_file) = match self.circuit_type {
            CircuitType::ECDSA => {
                let witness_dir = root.join("build/ecdsa/ecdsa_js");
                (witness_dir.join("ecdsa.r1cs"), witness_dir.join("ecdsa.wtns"))
            },
            CircuitType::JWT => {
                let witness_dir = root.join("build/jwt/jwt_js");
                (witness_dir.join("jwt.r1cs"), witness_dir.join("jwt.wtns"))
            },
        };
